...
const FRAME_PATH = `${import.meta.env.BASE_URL}frames/ezgif-frame-`
...
export default function ScrollImageSequence({ containerId = 'scroll-stage' }) {
  ...
  const findNearestLoadedImage = (requestedIndex) => {
    ...
    for (let offset = 1; offset < TOTAL_FRAMES; offset++) {
      ...
      if (
        down >= 1 &&
        statusRef.current[down] === 2 &&
        imagesRef.current[down]
      ) {
      ...
      if (
        up <= TOTAL_FRAMES &&
        statusRef.current[up] === 2 &&
        imagesRef.current[up]
      ) {
  ...
  const releaseDistantFrames = (centerFrame) => {
    ...
    for (let i = 1; i <= TOTAL_FRAMES; i++) {
      if (
        (i < keepMin || i > keepMax) &&
        statusRef.current[i] === 2
      ) {
        imagesRef.current[i] = null
        statusRef.current[i] = 0
      }
    }
  }
  ...
  const pumpQueue = () => {
    ...
    while (activeWorkersRef.current < CONCURRENT_LOADS) {
      ...
      img.onload = () => {
        ...
        if (
          index === cur ||
          (!hasDrawnFirstRef.current && index === 1)
        ) {
          renderFrame(cur)
  ...
  const handleScroll = () => {
    ...
    if (scrollableDist > 0) {
      ...
      progress = Math.max(
        0,
        Math.min(1, -rect.top / scrollableDist)
      )
    }
  ...
  const animate = () => {
    ...
    const frameToDraw = Math.max(
      1,
      Math.min(TOTAL_FRAMES, Math.round(currentFrameRef.current))
    )

    if (
      frameToDraw !== lastDrawnFrameRef.current ||
      !hasDrawnFirstRef.current
    ) {
      ...
      // Release distant frames occasionally
      // (hysteresis avoids thrashing when the scroll position hovers around
      // a retention boundary).
      if (
        Math.abs(
          frameToDraw - lastReleaseCenterRef.current
        ) >= 5
      ) {
  ...
  useEffect(() => {
    ...
    // Initialize queues:
    // 1. High priority: the first frames, so the hero appears instantly
    //    and early scrolling through Home stays smooth.
    const initialPriority = []

    for (let i = 1; i <= 40; i++) {
      initialPriority.push(i)
    }

    priorityQueueRef.current = initialPriority

    // 2. No eager whole-sequence preload. Frames load on demand around
    //    the current scroll position (see prioritizeWindow) which —
    //    together with the retention window above — keeps decoded memory bounded.
